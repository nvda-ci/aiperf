...
class BaseTransport(AIPerfLoggerMixin, ABC):
    """Base class for all transport protocol implementations.

    Transports handle the protocol layer (HTTP, gRPC, etc.) and must support:
    - JSON payloads
    - Custom headers
    - Streaming responses (or adapt to them)

    Subclasses should implement:
    - metadata(): Return transport capabilities
    - get_url(): Build base URL for requests
    - send_request(): Execute request and return record
    """
    ...
    def get_transport_headers(self, request_info: RequestInfo) -> Mapping[str, str]:
