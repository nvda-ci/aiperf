...
| `AIPERF_ZMQ_STREAMING_DEALER_YIELD_INTERVAL` | `1` | ≥ 0, ≤ 1000000 | Yield to the event loop after every X received messages from the ZMQ streaming dealer client. This is to prevent event loop starvation when a burst of messages is received. 1 means yield after every message, 100 means yield after every 100 messages, etc. 0 means disabled (never yield). |
| `AIPERF_ZMQ_STREAMING_ROUTER_YIELD_INTERVAL` | `1` | ≥ 0, ≤ 1000000 | Yield to the event loop after every X received messages from the ZMQ streaming router client. This is to prevent event loop starvation when a burst of messages is received. 1 means yield after every message, 100 means yield after every 100 messages, etc. 0 means disabled (never yield). |
| `AIPERF_ZMQ_REPLY_YIELD_INTERVAL` | `1` | ≥ 0, ≤ 1000000 | Yield to the event loop after every X received requests from the ZMQ router reply client. This is to prevent event loop starvation when a burst of requests is received. 1 means yield after every request, 100 means yield after every 100 requests, etc. 0 means disabled (never yield). |
| `AIPERF_ZMQ_REQUEST_YIELD_INTERVAL` | `1` | ≥ 0, ≤ 1000000 | Yield to the event loop after every X received responses from the ZMQ dealer request client. This is to prevent event loop starvation when a burst of responses is received. 1 means yield after every response, 100 means yield after every 100 responses, etc. 0 means disabled (never yield). |
| `AIPERF_ZMQ_PULL_YIELD_INTERVAL` | `1` | ≥ 0, ≤ 1000000 | Yield to the event loop after every X received messages from the ZMQ pull client. This is to prevent event loop starvation when a burst of messages is received. 1 means yield after every message, 100 means yield after every 100 messages, etc. 0 means disabled (never yield). |
| `AIPERF_ZMQ_SUB_YIELD_INTERVAL` | `1` | ≥ 0, ≤ 1000000 | Yield to the event loop after every X received messages from the ZMQ sub client. This is to prevent event loop starvation when a burst of messages is received. 1 means yield after every message, 100 means yield after every 100 messages, etc. 0 means disabled (never yield). |
| `AIPERF_ZMQ_PULL_MAX_CONCURRENCY` | `100000` | ≥ 1, ≤ 1000000 | Maximum concurrency for ZMQ PULL clients |
