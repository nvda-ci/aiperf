...
- KISS + DRY: minimal code, no premature abstractions, optimize for reader
...
**Commands:**
- `uv run pytest tests/unit/` - Fast, isolated, mock dependencies
- `uv run pytest -m integration` - Full system, real services

**Conventions:**
- `@pytest.mark.asyncio` for async tests, `@pytest.mark.parametrize` for data-driven
- `plugins.mock_plugin(PluginType.X, "name", MockClass)` for plugin mocking
...
uv run pytest -m component_integration # Component integration tests
