...
class ServerMetricsDataCollector(BaseMetricsCollectorMixin[ServerMetricsRecord]):
    ...
    - Uses @background_task for periodic collection (via mixin)
    ...
    def _process_simple_family(self, family) -> list[MetricSample]:
        ...
        return [
            MetricSample(labels=dict(label_tuple), value=value)
            for label_tuple, value in samples_by_labels.items()
        ]

    def _process_histogram_family(self, family) -> list[MetricSample]:
        """Process histogram metrics into structured format.

        Groups buckets, sum, and count by base labels (excluding 'le').
        ...
        # Convert to MetricSample list
        samples = []
        for label_tuple, hist_data in histograms.items():
            # Skip incomplete histograms
    ...
    def _process_summary_family(self, family) -> list[MetricSample]:
        """Process summary metrics into structured format.

        Groups quantiles, sum, and count by base labels (excluding 'quantile').
        ...
        for sample in family.samples:
            # Get base labels (without 'quantile')
