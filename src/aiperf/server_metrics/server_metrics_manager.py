...
class ServerMetricsManager(BaseComponentService):
    ...
    async def _profile_configure_command(
        ...
        for endpoint_url in self._server_metrics_endpoints:
            ...
            # Generate stable, collision-resistant collector ID using hash of URL
            url_hash = hashlib.sha256(endpoint_url.encode()).hexdigest()[:12]
            collector_id = f"collector_{url_hash}"
