...
class mock_plugin:
    ...
    Example:
        ...
        ...     endpoint_cls = get_class("endpoint", "test-endpoint")
    ...
    def __enter__(self) -> PluginEntry:
        ...
        if has_entry(self.category, self.name):
            self._previous_entry = get_entry(self.category, self.name)

        # Register the mock plugin
        register(self.category, self.name, self.cls, priority=self.priority)

        # Update metadata if provided
        entry = get_entry(self.category, self.name)
        if self.metadata:
            # Create new entry with updated metadata (PluginEntry is frozen)
            updated_entry = PluginEntry(
                category=entry.category,
                name=entry.name,
                ...
                loaded_class=entry.loaded_class,
            )
            _registry._types[self.category][self.name] = updated_entry
            return updated_entry

        return entry

    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        # Get current entry's class_path before cleanup
        current_entry = _registry._types.get(self.category, {}).get(self.name)
        current_class_path = current_entry.class_path if current_entry else None

        # Clean up class_path caches for the mock entry
        if current_class_path:
            _registry._type_entries_by_class_path.pop(current_class_path, None)
            # Clear any cached reverse lookups for this class_path
            cache_key = (self.category, current_class_path)
            _registry._class_path_to_name.pop(cache_key, None)

        # Restore previous entry or remove the mock
        if self._previous_entry is not None:
            _registry._types[self.category][self.name] = self._previous_entry
            # Restore class_path lookup for previous entry
            _registry._type_entries_by_class_path[self._previous_entry.class_path] = (
                self._previous_entry
            )
        elif (
            self.category in _registry._types
            and self.name in _registry._types[self.category]
        ):
            del _registry._types[self.category][self.name]
