...
def infer_unit_from_metric_name(metric_name: str) -> str:
    """Infer unit from metric name based on common Prometheus naming conventions.

    Prometheus metrics typically follow naming conventions that include units
    in the metric name (e.g., _seconds, _bytes, _total). This function extracts
    the unit based on these conventions.

    Args:
        metric_name: Name of the Prometheus metric (e.g., "http_request_duration_seconds")

    Returns:
        Inferred unit string (e.g., "s", "bytes", "%", "count") or empty string if unknown

    Examples:
        >>> infer_unit_from_metric_name("http_request_duration_seconds")
        "s"
        >>> infer_unit_from_metric_name("memory_usage_bytes")
        "bytes"
        >>> infer_unit_from_metric_name("http_requests_total")
        "count"
    """
