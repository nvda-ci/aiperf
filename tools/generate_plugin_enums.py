...
2. enums.pyi - Static type stubs for IDE autocomplete (gitignored, generated locally)

The enums.pyi file is NOT committed to the repository. Run this generator after
cloning or when adding 3rd-party plugins.
...
Usage:
    python tools/generate_plugin_enums.py [--check] [--py-only] [--pyi-only] [--runtime]

Options:
    --check     Check if enums.py is up-to-date (exit 1 if not)
    --py-only   Only generate enums.py
    --pyi-only  Only generate enums.pyi
    --runtime   Generate enums.pyi from actual loaded plugins (includes 3rd-party)
"""
...
def generate_enums_pyi_runtime() -> str:
...
"""
    ...
    for category in category_names:
        ...
        try:
            ...
            type_names = [entry.type_name for entry in type_entries]
        except Exception:
            type_names = []
...
def main() -> int:
    ...
    parser.add_argument(
        "--runtime",
        action="store_true",
        help="Generate enums.runtime.pyi from loaded plugins (includes 3rd-party)",
    )
    args = parser.parse_args()

    # Handle runtime generation separately (generates pyi from loaded plugins)
    if args.runtime:
        try:
            runtime_content = generate_enums_pyi_runtime()
            current = (
                ENUMS_PYI.read_text(encoding="utf-8") if ENUMS_PYI.exists() else ""
            )
            if normalize_content(current) != normalize_content(runtime_content):
                ENUMS_PYI.write_text(runtime_content, encoding="utf-8")
                print(f"Generated: {ENUMS_PYI} (with loaded plugins)")
            else:
                print("enums.pyi is already up-to-date")
            return 0
        except Exception as e:
            print(f"Error generating runtime stubs: {e}", file=sys.stderr)
            return 1
    ...
    if args.check:
        # Check mode: verify enums.py is up-to-date (pyi is gitignored, so not checked)
        errors = []
