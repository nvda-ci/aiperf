...
def generate_schemas() -> int:
    ...
    from aiperf.plugin.schema import CategoriesManifest, PluginsManifest, PluginSpec
    ...
    for name, spec in categories.items():
        ...
        if metadata_class := spec.get("metadata_class"):
            try:
                ...
                base_schema.setdefault("$defs", {}).update(
        ...
        base_schema.setdefault("$defs", {})[def_name] = entry_schema
    ...
    schemas = {
        "categories.schema.json": CategoriesManifest.model_json_schema(),
        "plugins.schema.json": base_schema,
    }
...
def generate_enums_py_content() -> str:
    ...
    categories = load_categories()
    builtin_plugins = {
        cat: [entry.name for entry, _ in plugins.iter_all(cat) if entry.is_builtin]
    ...
    for name, spec in categories.items():
        if not spec.get("enum"):
            continue
        types = builtin_plugins.get(name, [])
        if not types:
            continue
        ...
        examples = ", ".join(
            f"{enum_name}.{to_enum_member(n)}" for n in sorted(types)[:3]
        )
        lines.append(
...
def generate_enums_pyi_content() -> str:
    ...
    for category in sorted(plugins.list_categories()):
        desc = (
            plugins.get_category_metadata(category)["description"]
            .strip()
            .split("\n")[0]
        )
        lines.append(f'    {to_enum_member(category)} = "{category}"')
        lines.append(f'    """{desc}"""')
    ...
    lines.extend(
        [
            ...
            "    " + ", ".join(f'"{cat}"' for cat in plugins.list_categories()),
            "]",
            "",
        ]
    )
    ...
    for name, spec in categories.items():
        ...
        lines.append(f'    """Dynamic enum for {name} plugins."""')
        lines.append("")
        ...
        literal_name = f"{enum_name}Str"
        names = sorted(e.name for e in entries)
        quoted = ", ".join(f'"{n}"' for n in names)
        lines.append(f"{literal_name}: TypeAlias = Literal[{quoted}]")
        lines.append("")
