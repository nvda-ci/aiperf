...
Usage:
    python tools/generate_enum_stubs.py [--check] [--py-only] [--pyi-only]
...
def load_categories_with_sections() -> tuple[dict, dict[str, str]]:
    ...
    while i < len(lines):
        ...
        if re.match(r"^#\s*=+\s*$", line):
            # Potential section start - check next lines
            if i + 2 < len(lines):
                name_line = lines[i + 1]
                close_line = lines[i + 2]
                if re.match(r"^#\s+\S", name_line) and re.match(
                    r"^#\s*=+\s*$", close_line
                ):
                    ...
                    # Replace "Categories" or "Category" with "Types"
                    section_name = re.sub(
                        r"\s*Categor(y|ies)\s*$", " Types", section_name
                    )
                    current_section = section_name
                    i += 3
                    continue
...
def main() -> int:
    ...
    if args.check:
        ...
        if errors:
            ...
            print("Run: python tools/generate_enum_stubs.py", file=sys.stderr)
            return 1
