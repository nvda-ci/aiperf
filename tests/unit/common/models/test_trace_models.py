...
class TestBaseTraceData:
    ...
    def test_request_write_tracking(self, base_trace_timestamps):
        """Request write timestamps and sizes are tracked correctly."""
        ts = base_trace_timestamps
        write_timestamps = [
            ts["request_start"] + 1_000_000,
            ts["request_start"] + 2_000_000,
            ts["request_start"] + 3_000_000,
        ]
        write_sizes = [100, 200, 150]

        trace = create_base_trace_data(
            reference_time_ns=ts["reference_time"],
            reference_perf_ns=ts["reference_perf"],
            request_write_timestamps_perf_ns=write_timestamps,
            request_write_sizes_bytes=write_sizes,
        )

        assert trace.request_write_timestamps_perf_ns == write_timestamps
        assert trace.request_write_sizes_bytes == write_sizes

    def test_response_receive_tracking(self, base_trace_timestamps):
        """Response receive timestamps and sizes are tracked correctly."""
        ts = base_trace_timestamps
        receive_timestamps = [
            ts["response_start"] + 1_000_000,
            ts["response_start"] + 5_000_000,
        ]
        receive_sizes = [512, 1024]

        trace = create_base_trace_data(
            reference_time_ns=ts["reference_time"],
            reference_perf_ns=ts["reference_perf"],
            response_receive_timestamps_perf_ns=receive_timestamps,
            response_receive_sizes_bytes=receive_sizes,
        )

        assert trace.response_receive_timestamps_perf_ns == receive_timestamps
        assert trace.response_receive_sizes_bytes == receive_sizes
    ...
    def test_trace_type_is_frozen(self):
        """Trace type field is frozen and cannot be modified after creation."""
        trace = create_base_trace_data()
...
class TestTraceDataExport:
    ...
    def test_export_converts_timestamp_lists(self, base_trace_timestamps):
        """Export converts lists of timestamps correctly."""
        ts = base_trace_timestamps
        write_timestamps = [
            ts["request_start"] + 1_000_000,
            ts["request_start"] + 2_000_000,
        ...
        assert len(export.request_write_timestamps_ns) == 2
        expected_first = ts["reference_time"] + (
            write_timestamps[0] - ts["reference_perf"]
        )
        assert export.request_write_timestamps_ns[0] == expected_first
    ...
    def test_computed_waiting_duration(self, base_trace_timestamps):
        ...
        ts = base_trace_timestamps
        first_response = ts["response_start"]

        trace = create_base_trace_data(
            reference_time_ns=ts["reference_time"],
            reference_perf_ns=ts["reference_perf"],
            request_send_end_perf_ns=ts["request_end"],
            response_receive_timestamps_perf_ns=[first_response],
        )

        export = trace.to_export()

        expected_waiting = first_response - ts["request_end"]
    ...
    def test_computed_receiving_duration_multiple_chunks(self, base_trace_timestamps):
        ...
        receive_timestamps = [
            ts["response_start"],
            ts["response_start"] + 10_000_000,
            ts["response_start"] + 20_000_000,
        ]
        ...
        expected_receiving = receive_timestamps[-1] - receive_timestamps[0]
    ...
    def test_computed_receiving_duration_single_chunk(self, base_trace_timestamps):
        ...
        trace = create_base_trace_data(
            reference_time_ns=ts["reference_time"],
            reference_perf_ns=ts["reference_perf"],
            response_receive_timestamps_perf_ns=[ts["response_start"]],
        )
    ...
    def test_computed_total_duration(self, base_trace_timestamps):
        ...
        assert export.total_ns == expected_total
    ...
    def test_computed_fields_none_when_data_missing(self):
        ...
        assert export.total_ns is None
    ...
    def test_export_serialization_includes_computed_fields(self, base_trace_timestamps):
        ...
        assert "total_ns" in export_dict
...
class TestAioHttpTraceDataExport:
    ...
    def test_export_uses_discriminator_routing(self, aiohttp_trace_timestamps):
        ...
        assert type(export) == AioHttpTraceDataExport
...
class TestTraceDataEdgeCases:
    ...
    def test_empty_write_and_receive_lists(self):
        """Empty write/receive lists are handled correctly."""
        trace = create_base_trace_data(
            request_write_timestamps_perf_ns=[],
            request_write_sizes_bytes=[],
            response_receive_timestamps_perf_ns=[],
            response_receive_sizes_bytes=[],
        )

        assert trace.request_write_timestamps_perf_ns == []
        assert trace.response_receive_timestamps_perf_ns == []

        export = trace.to_export()
        assert export.request_write_timestamps_ns == []
        assert export.receiving_ns is None

    def test_mismatched_timestamp_and_size_lists(self):
        """Mismatched timestamp and size lists are stored as-is."""
        trace = create_base_trace_data(
            request_write_timestamps_perf_ns=[1000, 2000, 3000],
            request_write_sizes_bytes=[100, 200],  # Fewer sizes than timestamps
        )

        assert len(trace.request_write_timestamps_perf_ns) == 3
        assert len(trace.request_write_sizes_bytes) == 2
