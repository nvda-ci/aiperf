...
class TestAsyncKeyedDuplicateTrackerConcurrency:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_tasks,writes_per_task", [(3, 10), (5, 5), (10, 3)])
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_keys", [2, 3, 5])
...
class TestAsyncKeyedDuplicateTrackerEdgeCases:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "key", ["", "key-with-dashes", "key.with.dots", "key/with/slashes"]
    )
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_alternations", [3, 6, 10])
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("num_duplicates", [10, 100, 1000])
...
class TestAsyncKeyedDuplicateTrackerFlush:
    ...
    async def test_flush_with_pending_duplicates(
        ...
        assert to_flush[0].value == 1  # Record value
    ...
    async def test_flush_multiple_keys(
        ...
        flushed_values = {record.value for record in to_flush}
        assert flushed_values == {1, 2}
