...
class TestPluginRegistryBasics:
    ...
    def test_load_manifest(self, registry, temp_registry_file):
        """Test loading built-in registry from YAML."""
        registry.load_manifest(temp_registry_file)
    ...
    def test_load_manifest_nonexistent_file(self, registry):
        ...
        with pytest.raises(RuntimeError, match="Built-in plugins.yaml not found"):
            registry.load_manifest("/nonexistent/path/plugins.yaml")

    def test_load_manifest_priority_conflict(self, registry):
    ...
    def test_discover_plugins(self, registry, temp_registry_file):
        ...
        # Use the actual temp file name in the entry point value
        mock_ep.value = f"test.module:{temp_registry_file.name}"
        mock_ep.dist = None

        # Mock importlib.util.find_spec to return a mock spec
        mock_spec = Mock()
        mock_spec.submodule_search_locations = [str(temp_registry_file.parent)]

        with (
            patch("aiperf.plugin.plugins.entry_points", return_value=[mock_ep]),
            patch(
                "aiperf.plugin.plugins.importlib.util.find_spec",
    ...
    def test_load_manifest_with_string_path(self, registry, temp_registry_file):
        """Test loading registry with string path instead of Path."""
        registry.load_manifest(str(temp_registry_file))
...
class TestIterAll:
    """Tests for iter_all() method."""

    def test_iter_all_returns_tuples(self, registry, temp_registry_file, mock_class):
        """Test that iter_all returns (PluginEntry, type) tuples."""
        registry.load_manifest(temp_registry_file)
        ...
        with patch("importlib.import_module") as mock_import:
            ...
            entries = list(registry.iter_all("endpoint"))

            assert len(entries) == 2
            assert all(isinstance(e, PluginEntry) for e, _ in entries)

    def test_iter_all_sorted_alphabetically(
        ...
        with patch("importlib.import_module") as mock_import:
            ...
            assert entries[0][0].name == "low_priority_endpoint"
            assert entries[1][0].name == "test_endpoint"

    def test_iter_all_provides_loaded_class(
        self, registry, temp_registry_file, mock_class
    ):
        """Test that iter_all provides already-loaded classes."""
        registry.load_manifest(temp_registry_file)
        ...
        with patch("importlib.import_module") as mock_import:
            ...
            for entry, cls in registry.iter_all("endpoint"):
                assert entry.name
                assert entry.description
                assert entry.priority >= 0
                assert entry.class_path
                # Class is already loaded by iter_all
                assert cls is mock_class

    def test_iter_all_unknown_category(self, registry):
        """Test that unknown category returns empty iterator."""
        entries = list(registry.iter_all("nonexistent_category"))
        assert entries == []

    def test_iter_all_no_category_returns_all(
        self, registry, temp_registry_file, mock_class
    ):
        """Test that iter_all() without category returns all plugins."""
        registry.load_manifest(temp_registry_file)
        ...
        with patch("importlib.import_module") as mock_import:
            ...
            mock_module.TestEndpoint = mock_class
            mock_module.LowPriorityEndpoint = mock_class
            mock_module.TestProcessor = mock_class
            mock_module.OptionalProcessor = mock_class
            mock_import.return_value = mock_module

            entries = list(registry.iter_all())

            # Should have plugins from all categories
            assert len(entries) >= 2
...
class TestImplicitPriority:
    ...
    def test_equal_priority_package_beats_builtin(self, registry):
        ...
        registry.load_manifest(builtin_path)
        registry.load_manifest(package_path)
    ...
    def test_lower_priority_package_loses(self, registry):
        ...
        registry.load_manifest(path_a)
        registry.load_manifest(path_b)
...
class TestGlobalRegistry:
    ...
    def test_module_level_reset(self):
        """Test that plugins.reset_registry() clears custom types."""
        # Load a custom type
        registry_data = {
            "plugin": {"name": "test", "version": "1.0.0"},
            "endpoint": {
        ...
        plugins._registry.load_manifest(path)
    ...
    def test_module_level_iter_all(self):
        """Test that plugins.iter_all() works at module level."""
        registry_data = {
            "plugin": {"name": "test", "version": "1.0.0"},
            "endpoint": {
        ...
        # Test module-level iter_entries (doesn't load classes)
        entries = list(plugins.iter_entries("endpoint"))
        assert len(entries) > 0

        # Test iter_all with mocked import
        with patch("importlib.import_module") as mock_import:
            mock_module = Mock()
            mock_module.TestClass = type("TestClass", (), {})
            mock_import.return_value = mock_module

            entries = list(plugins.iter_all("endpoint"))
            assert len(entries) > 0
...
class TestModuleLevelFunctions:
    ...
    def test_load_manifest_module_level(self):
        """Test load_manifest at module level."""
        yaml_content = {
...
class TestIntegration:
    ...
    def test_lazy_loading_only_on_demand(
        ...
        # iter_all DOES import since it returns the loaded class
        with patch("importlib.import_module") as mock_import:
            mock_module = Mock()
            mock_module.TestEndpoint = mock_class
            mock_module.LowPriorityEndpoint = mock_class
            ...
            entries = list(registry.iter_all("endpoint"))
            assert len(entries) == 2
            # iter_all imports all classes
            assert mock_import.call_count >= 1

        # get() SHOULD import (or use cached class)
        with patch("importlib.import_module") as mock_import:
            mock_module = Mock()
            mock_module.TestEndpoint = mock_class
            mock_import.return_value = mock_module
    ...
    def test_full_lifecycle(self, registry):
        ...
        # 3. Iterate types
        entries = list(registry.iter_all("endpoint"))
        type_names = [entry.name for entry, _ in entries]
...
class TestEdgeCases:
    ...
    def test_discover_plugins_path_object(self, registry):
        """Test discover_plugins with Path object return via importlib.util.find_spec."""
        yaml_content = {
        ...
        # Use the actual temp file name in the entry point value
        mock_ep.value = f"path.module:{path.name}"
        mock_ep.dist = None

        # Mock importlib.util.find_spec to return a mock spec
        mock_spec = Mock()
        mock_spec.submodule_search_locations = [str(path.parent)]
